...
#appending current date and time to the outputfile
now = str(datetime.now().strftime('_%Y_%m_%d-%I_%M_%S_%p')) #Windows doesn't use colons,semicolons, hence those will cause error within the path
...
class MultipleSearchEngines(object):
    ...
    def output(self, output=out.PRINT, path=None): # Actual output/write code
        ...
        if out.HTML in output:
            out.write_file(out.create_html_data(self._engines), path + now + u'.html') # now variable is used for current date and time
        if out.CSV in output:
            out.write_file(out.create_csv_data(self._engines), path + now + u'.csv') # now variable is used for current date and time
        if out.JSON in output:
            out.write_file(out.create_json_data(self._engines), path + now + u'.json') # now variable is used for current date and time
